...
Output files:
RESULTS/XX		Array of BRDF values
RESULTS/XX.log		Log showing albedo, component absorption and nadir reflectance per waveband
...
#define BIN_OUT 1  /*  MW:  ** Binary image output, for reading with ENVI **   */
#define FLOAT_OUT 0  /*  MW:  ** Floating point image output, for reading with IDL **   */
...
#define RESULT_ZEN_GRID 10   //MW: for bins of size N degrees:  RESULT_ZEN_GRID = (90/N)+1
#define RESULT_AZ_GRID 36    //MW: for bins of size N degrees:  RESULT_AZ_GRID = (360/N)
...
void trace(photon,npos,current_st_no)
...
void lambertian_interact(stand,photon)
	struct stand *stand;
	struct photon *photon;
...
void foliage_interact(vec,newvec,photon)
...
int intersect_facets_crowns(no_crowns)
	int no_crowns;
...
void leaf_reflectance_fn(in_photon_vec,out_photon_vec,leaf_normal_vec,RHO_AR,TAU_AR,OUT_AR)
...
void phase_fn_evaluate(theta_s,theta_o,phi,RHO_AR,TAU_AR,OUT_AR)
	float theta_s,theta_o,phi;
	float RHO_AR[MAX_NO_WVBANDS],TAU_AR[MAX_NO_WVBANDS],OUT_AR[MAX_NO_WVBANDS];
...
void mk_phase_fn_array(RHO_AR,TAU_AR,OUT_AR)
...
void light_incident_on_facet(stand,pos,ftype,facet_norm,vec_old,bounce_no,radsum,radsum2)
	struct stand *stand;
...
void read_struct_data_photosyn()
...
void facet_radiance(stand,pos,facet_norm,vec,type,radsum,radsum2)
...
struct photon *find_reflectance(stand,photon)
	struct stand *stand;
	struct photon *photon;
{
	...
	npos=mkpoint(0.0,0.0,0.0);
	type=oned_find_intersection(stand,photon->pos,photon->vec,vec_old,0,npos);
	...
	facet_radiance(stand,npos,facet_norm,photon->vec,type,radsum,radsum2);
	/* reflectance is be given by radiance/incoming_flux */



	for (i=0;i<NO_WVBANDS;i++) photon->intensity[i]=radsum[i][0]/(cos(degtorad(SOLAR_ZENITH))*SOLAR_RAD[i]+SKY_RAD[i]);
		...
		scattering order (e.g. BOUNCE_THRESHOLD=5) **/
{
	...
	photon=mkphoton(mkpoint(0.0,0.0,0.0),mkpoint(0.0,0.0,0.0),ph_intensity);
	...
	for(j=0;j<N_LUE_SAMPLES;j++) {						/* n samples of random leaf within canopy */
		...
		if (ONED_FLAG==1) {
			...
			while (stand_no<0) {
				...
				for  (k=0;k<NOSTANDS;k++) if (inside_stand(photon->pos,k)) stand_no=k;
...
void threed_facet_radiance(stand_no,pos,facet_norm,vec,type,radsum0,radsum,radsum2,INSIDE_STAND)
...

{
	...
	if (NOFACETS >0) {
		...
		for (i=0;i<NOFACETS;i++) {
			fscanf(fpfacet,"%f %f %f %f %f %f %f %f %f %s",&x1,&y1,&z1,&x2,&y2,&z2,&x3,&y3,&z3,&filename[70]);
...

{
	...
	char filename[70];
	struct point *phvec;
	struct photon *photon, *out_photon;
	...
	if ((MODE=='i') || (MODE=='s')) {
		...
		if (MODE =='r') {
			...
			read_reverse_data();
			printf("Reading data for reverse mode... DONE\n");
		} 
...
for (VIEW_ZENITH=VIEW_ZENITH_START; VIEW_ZENITH<=VIEW_ZENITH_END; VIEW_ZENITH=VIEW_ZENITH+VIEW_ZENITH_STEP) {
	for (VIEW_AZIMUTH=VIEW_AZIMUTH_START; VIEW_AZIMUTH<=VIEW_AZIMUTH_END; VIEW_AZIMUTH=VIEW_AZIMUTH+VIEW_AZIMUTH_STEP) {
			...
			while (PHOTON_COUNT < NO_PHOTONS_EFF) {
				for (yst=-1.0*Y_DIM+stepsize*0.5+off_y;yst<=(Y_DIM+off_y);yst=yst+stepsize) {
					for (xst=-1.0*X_DIM+stepsize*0.5+off_x;xst<=(X_DIM+off_x);xst=xst+stepsize) {
						if ( (float)((5*PHOTON_COUNT)/NO_PHOTONS_EFF)==(5.0*((float)(PHOTON_COUNT))/((float)NO_PHOTONS_EFF))) {
							printf("%5.1f %% paths traced\n",100.0*(float)(PHOTON_COUNT)/(float)(NO_PHOTONS_EFF));  
						}
						photon->pos->x=xst;
						photon->pos->y=yst;
						photon->pos->z=CANOPY_TOP_IN;
						if (MODE=='f') {
							if (DIFF_SKY_FLAG==1) photon->vec=rand_norm_vec_down(photon->vec);
							else {
							...
							if (VIEW_ZENITH<0) photon->vec=rand_norm_vec_down_albedo(photon->vec);
							else {
						...
						for (i=0;i<NO_WVBANDS;i++) photon->intensity[i]=1.0;
						...
						/*
						   ...
						   if (DIFF_SKY_FLAG==1) 
						   for (i=0;i<NO_WVBANDS;i++)
						   photon->intensity[i]=norm_skyfac(i,photon->vec->theta,photon->vec->phi);
						   */
						...
						if ((MODE == 'r') && (ONED_FLAG ==1)) {
							photon->pos->z=TOP_ONED;
							out_photon=find_reflectance(STANDS[0],photon);
							for (i=0;i<NO_WVBANDS;i++) {
						...
						if ((MODE == 'r') && (ONED_FLAG ==0)) {
							...
							INSIDE_STAND=0;
							out_photon=threed_find_reflectance(SKY_PLANE_NUMBER,photon,INSIDE_STAND);
							for (i=0;i<NO_WVBANDS;i++) {
								mean_ref[i]+=out_photon->intensity[i];
								fc_int[COMPNO][i]+=out_photon->intensity[i];
							}
						...
						else if (ONED_FLAG ==0) out_photon=sim(photon); 
						else {
							photon->pos->z=TOP_ONED;
							prob=rand()*RAND_MULT;
							if (prob<=FRAC_COV) out_photon=oned_interact(STANDS[0],photon);
							else { out_photon=ground_interact(photon);COMPNO=0; }
						}
						out_no=COMPNO;
						...
						if ( (MODE =='r') || ((phvec->theta <PI*0.5) && (phvec->theta >=0.0) && (resulterrflag==0))) {
							...
							if (index_theta==0) index_phi=0;
								...
								else index_phi=(int)(0.5*(phvec->phi+temp)/PI*((float)RESULT_AZ_GRID-1.0));
							}
							if (MODE=='f') {
								...
								for (i=0;i<NO_WVBANDS;i++) {
									...
									result[i][index_theta][index_phi][DIFF_SKY_FLAG]=result[i][index_theta][index_phi][DIFF_SKY_FLAG]+intensity;
								}
							} else {
								fc_result[out_no]++;
								fprintf(fpim,"%d\n",out_no);
		...
		/* free(photon->intensity);free(photon->vec);free(photon);	free(out_photon->intensity);free(out_photon->vec);free(out_photon); */
